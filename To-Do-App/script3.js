...
add_task.addEventListener("click", () => {
  ...
  const random_id = task.id;
  localStorage.setItem(random_id, JSON.stringify(task.innerText));

  delete_task(delete_button, task, task_row);
  edit_task(
    edit_button,
    task,
    write_task,
...
function delete_task(delete_button, task, task_row) {
  delete_button.addEventListener("click", () => {
    task_row.remove();
    console.log("task id: ", task.id);
    console.log("task id: ", task.className);
    // localStorage.removeItem(1749570391330);

    for (let i = 0; i < localStorage.length; i++) {
      // console.log(localStorage.key(i));

      if (task.id == localStorage.key(i)) {
...
function save_button_working(
  ...
  save_button.addEventListener("click", () => {
    ...
    edit_local(task.innerText, task);
  });
}
...
function edit_local(write, task) {
  for (let i = 0; i < localStorage.length; i++) {
    if (task.id == localStorage.key(i)) {
      console.log("Ediiiiiit", task.id);
      console.log("Ediiiiiit22222", localStorage.key(i));

      localStorage.setItem(task.id, JSON.stringify(write));
    }
  }
}
